...
function scenarioloader(source) {
  ...
  function replacer(match, fromStatement, obj, quote, filename) {
    ...
    const metadataToImportStatement = data => `import * as ${data.moduleName} from ${data.fileName};\n`;
    const metadataToString = data => `{fileName: ${data.fileName}, result: ${data.moduleName}},\n`;
